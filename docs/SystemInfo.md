...
**kernelMemoryTCP** | **Bool** | Indicates if the host has kernel memory TCP limit support enabled. This field is omitted if not supported.  Kernel memory TCP limits are not supported when using cgroups v2, which does not support the corresponding &#x60;memory.kmem.tcp.limit_in_bytes&#x60; cgroup.  | [optional] 
...
**cgroupVersion** | **String** | The version of the cgroup.  | [optional] [default to ._1]
